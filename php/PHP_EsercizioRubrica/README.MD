...
1. Esegui composer install nella root del progetto:
   ```bash
   composer install
   ```

2. Assicurati di avere installato Docker, in alternativa puoi configurare il server PHP locale di (ad es. [XAMPP](https://www.apachefriends.org/it/index.html).

3. Se esegui con docker, esegui il comando:
   ```bash
   docker-compose up -d
   ```
